...
import { useZeroPollingMFS100 } from "@/hooks/useZeroPollingMFS100";

interface ZeroPollingMFS100CaptureProps {
  index: number;
...
export function ZeroPollingMFS100Capture({ 
  index, 
  onCaptureSuccess,
  onCaptureError,
  disabled = false,
  ...
  targetQuality = 60
}: ZeroPollingMFS100CaptureProps) {
  const [capturedImage, setCapturedImage] = useState<string>("");
  const [captureQuality, setCaptureQuality] = useState<number | null>(null);
  
  const { isCapturing, lastError, captureFingerprint } = useZeroPollingMFS100();

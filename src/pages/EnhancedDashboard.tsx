...
import { Users, GraduationCap, Shield, Activity, AlertTriangle, CheckCircle, Clock, Database, Wifi, RefreshCw } from 'lucide-react';
...
export default function EnhancedDashboard() {
  ...
  const {
    ...
    queryFn: async () => {
      const [studentsRes, batchesRes] = await Promise.all([supabase.from('students').select('id, created_at, batch_id').eq('is_enabled', true), supabase.from('batches').select('id, created_at, max_students').eq('is_enabled', true)]);
      ...
      const batchUtilization = await Promise.all(batches.map(async batch => {
        const {
          count
      ...
      return {
        ...
        batchUtilization,
        avgUtilization: Math.round(batchUtilization.reduce((sum, b) => sum + b.utilization, 0) / batchUtilization.length || 0)
      };
    },
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            performHealthCheck();
            refetchStats();
          }} disabled={isChecking} className="gap-2">
        ...
        {metrics && <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <Card>
        ...
        {/* Batch Utilization */}
        {stats?.batchUtilization && stats.batchUtilization.length > 0 && <Card>
            <CardHeader>
              <CardTitle>Batch Utilization</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {stats.batchUtilization.map((batch, index) => <div key={batch.id} className="space-y-2">
        ...
        {/* Security Alert */}
        {metrics?.overall === 'critical' && <Alert className="border-red-200 bg-red-50">

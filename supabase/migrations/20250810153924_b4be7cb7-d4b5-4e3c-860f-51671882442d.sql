...
USING (
  ...
  (
    ...
    (auth.uid() = user_id) OR 
    public.user_has_batch_access(auth.uid(), id)
  )
);
...
USING (
  ...
  (
    public.is_super_admin() OR 
    (auth.uid() = user_id) OR 
    (user_id IS NULL AND public.user_has_batch_access(auth.uid(), id))
  )
);
...
USING (
  ...
  (
    public.is_super_admin() OR 
    (auth.uid() = user_id) OR 
    public.user_has_batch_access(auth.uid(), batch_id)
  )
);
...
USING (
  ...
  (
    public.is_super_admin() OR 
    (auth.uid() = user_id) OR 
    (user_id IS NULL AND public.user_has_batch_access(auth.uid(), batch_id))
  )
);

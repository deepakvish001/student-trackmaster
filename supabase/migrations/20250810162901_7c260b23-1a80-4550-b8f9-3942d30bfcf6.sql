-- Create optimized indexes for ultra-fast queries (fixed version)
...
-- Fingerprint availability check using hash for performance
CREATE INDEX IF NOT EXISTS idx_students_finger1_hash ON students(md5(finger_1)) WHERE finger_1 IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_students_finger2_hash ON students(md5(finger_2)) WHERE finger_2 IS NOT NULL;
